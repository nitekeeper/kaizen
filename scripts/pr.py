...
def load_run_context(
    ...
    try:
        ...
        if row is None:
            raise RuntimeError(
                f"No project found for run {run_id} "
        ...
        if cycle_ids:
            ...
            cur = conn.execute(
                f"SELECT * FROM abandonments WHERE cycle_id IN ({placeholders}) "
                f"ORDER BY cycle_id",
                cycle_ids,
            )
            rows = cur.fetchall()
...
def render_pr_body(
    ...
    parts.append(
        f"\U0001F916 Generated by Kaizen against "
        f"{project['name']} at {project['git_url']}"
    )
...
def open_pr(
    ...
    if not lines:
        raise RuntimeError(
            "gh pr create returned exit 0 but no URL on stdout"
        )
    return lines[-1]
