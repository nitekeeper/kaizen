...
| `KAIZEN_TRANSPORT` | `host` | Dispatch transport (M8). `host` (unset/empty — the DEFAULT as of M8c) — in **subagent** mode, Phase 4 (+ the review→fix loop + commit) is delegated to atelier's in-process host engine via `scripts/host_cycle_entry.py` instead of being run by prose; the Phase 1-3 meeting still runs in-prose and hands its Action-Items DAG to the engine. `bridge` — the EXPLICIT opt-out (`KAIZEN_TRANSPORT=bridge`): the legacy SQLite queue-bridge / prose cycle, unchanged and slated for removal in M8c-2. Resolved by `scripts/transport.py` (single source of truth); a typo fails loud (`UnknownTransportError`). `--mode team` is bridge-only — set `KAIZEN_TRANSPORT=bridge` to use team mode; `host` is wired at the subagent-SKILL layer only (see `internal/cycle/SKILL.md` Phase 4 transport fork). |
