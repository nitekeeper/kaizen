...
Cycles run via fire-and-forget `Agent` tool calls (subagent mode); Phase 4 + the review→fix loop + commit are delegated to atelier's in-process host engine (see `KAIZEN_TRANSPORT` below).

## Loom comms (F16)

When the Loom agent-chat server is available, loom-agent-chat inter-agent comms are **MANDATORY** — `KAIZEN_LOOM_COMMS=0` is the only opt-out, and loom errors never abort a run. Detection and wiring are automatic: follow the "Loom comms — MANDATORY when available (F16)" section in `internal/cycle/SKILL.md` (detect via `PYTHONPATH=. python3 scripts/loom_comms.py detect`; per-agent block via `PYTHONPATH=. python3 scripts/loom_comms.py block --role <role> --channel <chan>`). Subagent completion replies are unchanged.
...
### Step 3 — Route to the run procedure
...
When Step 3 returns, surface the summary to the user. It should include:
...
|---|---|---|
| `KAIZEN_TRANSPORT` | `host` | Dispatch transport (M8). `host` (unset/empty — the DEFAULT) — Phase 4 (+ the review→fix loop + commit) is delegated to atelier's in-process host engine via `scripts/host_cycle_entry.py` instead of being run by prose; the Phase 1-3 meeting still runs in-prose and hands its Action-Items DAG to the engine. Resolved by `scripts/transport.py` (single source of truth); a typo fails loud (`UnknownTransportError`). Wired at the subagent-SKILL layer (see `internal/cycle/SKILL.md` Phase 4 transport fork). |
