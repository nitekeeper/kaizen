...
| `KAIZEN_TRANSPORT` | `bridge` | Dispatch transport (M8). `bridge` (unset/empty) — the SQLite queue-bridge / prose cycle, unchanged. `host` — in **subagent** mode, Phase 4 (+ the review→fix loop + commit) is delegated to atelier's in-process host engine via `scripts/host_cycle_entry.py` instead of being run by prose; the Phase 1-3 meeting still runs in-prose and hands its Action-Items DAG to the engine. Resolved by `scripts/transport.py` (single source of truth); a typo fails loud (`UnknownTransportError`). `--mode team` is bridge-only — `host` is wired at the subagent-SKILL layer only (see `internal/cycle/SKILL.md` Phase 4 transport fork). |
