...
def test_orchestrate_run_with_run_id_writes_failed_sentinel_on_cycle_exception(
    ...
    """The run_id (create-run-only) entry path's failure case: when the cycle
    loop raises, runs.branch must transition from '<pending>' (or the real name)
    to '<failed>' so render_pr_body refuses any later PR attempt."""
    _install_orchestrator_stubs(monkeypatch, tmp_path)
