...
- `{"available": true, "client": "<path>", ...}` (exit 0) → loom comms are mandatory for this cycle. **Read `internal/cycle/loom-comms.md` and follow the mandatory loom procedure there** (canonical channel, register/create-or-join, per-dispatch block, between-phase reads, run-end deregister).
